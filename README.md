...
下一步推荐 → **FE-S2-002（打字机 + Markdown 增量解析，0.5d）**：当前 chat 骨架已就位但 LLM 输出还是裸文本, FE-S2-002 把 `marked` / `markdown-it` 增量解析挂上去 + 打字机节流（避免 token 100 个/s 触发 100 次 reflow）+ `[1] [2]` inline citation reference 转可点击 anchor（为 FE-S2-003 抽屉打入口）。或并行 **FE-S2-003**（引用源 ActionSheet 抽屉, 0.5d）也可以, 两者无依赖；**FE-S2-004**（VIP 升级支付通道 + 配额引导精修, 0.5d）也可以接上去。剩余战场：**FE-S2-002/003/004**（前端 3 PR, ~1.5d）。
