...
下一步推荐 → **FE-S2-001（AI 对话页 UI + Pinia store + SSE consume，1d）** 或并行 **QA-S2-001（Agent E2E 集成测试，1d）**：后端 P0 主链路 BE-S2-001/002/003/004/005/006a/006b/007/008/009 全部 ✅，前端依赖已就绪；FE-S2-001 PR 内会落地：H5 + MP-WEIXIN 兼容的 SSE 消费（uni.connectSocket fallback / `onChunkReceived` 事件流）+ Pinia store 多轮会话状态机（pending → streaming → done / error）+ 错误兜底（429 弹 VIP 升级 modal / 网络断重试 / disclaimer 渲染）。剩余战场：**FE-S2-001/002/003/004**（前端 4 PR，对话页 + 打字机 + 引用源面板 + 配额引导）+ **QA-S2-001/002**（E2E 集成测试 + RAG 评测 CI 化）, 共 ~3.5d。
