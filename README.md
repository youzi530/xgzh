...
- ✅ **BE-S2-006b**：余下 3 工具（peers / sentiment / historical）+ hybrid_search Tool 化
- ✅ **BE-S2-007**：LangGraph 主循环 + 引用源装配 + 端层 disclaimer
...
下一步推荐 → **BE-S2-009（评测集 80 条 + 离线评测脚手架，1d）**：BE-S2-007 ✅ + BE-S2-008 ✅ 双依赖齐备, Agent 主线全打通，到了"用 80 条标注 query 把召回@5 / 幻觉率 / LLM-as-judge 三指标量化"的时点。BE-S2-009 PR 内会落地：`eval/dataset/sprint2_80q.jsonl`（80 条 = IPO 基本面 20 + 风险点 20 + 同业对比 20 + RAG 召回 20）+ `eval/run_eval.py`（离线脚手架，跑分 + 报告 markdown）+ `Makefile eval-sprint2` 一键 + spec/09 §评测口径补充。
