...
- **后端测试**：
  ...
  - 有 DB：`make test-all` ⇒ **411 passed in ~35s**（373 → 411，新增 38 条 BE-S2-006b 测试：`tests/test_sentiment_tool.py` 7（placeholder 形状 / code 归一 / window_days 默认 / 越界 / OpenAI schema）+ `tests/test_hybrid_search_tool.py` 9（显式 session 注入 + ipo_code 大写归一 + UUID→str 序列化 / 不传 session 走 factory / 空结果 warning / lang+doc_type 透传 / 入参校验 / 上游异常 / OpenAI schema）+ `tests/integration/test_peers_tool.py` 9（l2 优先排序 + 排除自己 / l1 fallback / 无可比 + warning / target 不存在 → failure / 自定义 dimensions 子集 / Literal 拒收 / OpenAI schema）+ `tests/integration/test_historical_tool.py` 13（industry/sponsor/year_range 各种过滤 + 全市场 / 命中 0 warning / 全 NULL warning / 排除非 listed / year_range 长度/顺序/越界校验 / OpenAI schema）；累计 11 张表 + 0004 tsvector 全文索引 + 混合检索 vector+BM25+RRF+reranker + Tool 注册中心 + 沙盒 + **6 个 Tool 全数注册到位**全链路）
...
- ✅ **BE-S2-006a**：Tool 注册中心 + 2 个最简工具 ✅
- ✅ **BE-S2-006b**：余下 3 工具（peers / sentiment / historical）+ hybrid_search Tool 化 ✅（本 PR）
...
下一步推荐 → **BE-S2-007（LangGraph 主循环 + 引用源装配 + 端层 disclaimer，1.5d）**：6 个 Tool 全数注册到位，BE-S2-002 facade ✅ + BE-S2-005 hybrid_search ✅ + BE-S2-006a/b ✅ 三依赖齐备。BE-S2-007 PR 内会落地：`agent/graph.py`（LangGraph StateGraph 主循环）+ `agent/citation.py`（引用源装配）+ `app/api/v1/chat.py`（POST /v1/chat/diagnose SSE 端层 + 写 chat_token_usage / chat_tool_calls + 强制 DISCLAIMER）+ E2E 集成测试。
