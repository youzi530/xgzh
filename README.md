...
- ✅ **FE-S2-003**（引用源底部抽屉 + 跨端原文 PDF 跳转）：把 FE-S2-002 留下的"`[N]` chip 已挂 `@tap` 但只弹 modal 占位"升级为 **底部抽屉 (Bottom Sheet) + 跨端跳原文 PDF**：用户点 `[N]` → 抽屉滑起 → 完整 snippet + 元信息（页码 / 相关度 / chunk_id 短哈希）+ "查看原文 PDF" + "复制片段" + 多引用 ‹ 1/3 › 切换；prospectus_url 走 `IPODetail` lazy-fetch + 模块内 `Map` 缓存防重复请求；跨端 PDF 打开 H5 `window.open` / MP-WEIXIN `wx.downloadFile + wx.openDocument` / App `plus.runtime.openURL` 三支齐活，全失败兜底"复制 URL + toast"。**3 文件改动**：① **`apps/mp/components/CitationDrawer.vue`**（新建 +390）抽屉 UI + 7 段布局（mask / panel / handle / header / meta chips / scroll-view body / actions row）+ 5 props（`visible` / `citations` / `activeIdx` / `ipoName?` / `prospectusUrl?: string|null|undefined`）+ 4 emits（`update:visible` / `update:activeIdx` / `open-prospectus` / `ensure-prospectus`）+ keyframe `cd-slide-up` 0.22s 滑入（不用 Vue `<transition>` 因 MP-WEIXIN 兼容坑，v-if 直接挂 / 卸更稳定）+ watch `[visible, activeCitation.ipo_code]` 触发 `ensure-prospectus`（让父页 lazy-fetch IPODetail）+ prospectus 按钮三态文案（disabled = "原文暂未入库" / loading = "加载中…" / ready = "查看原文 PDF"）+ chunk_id 取前 8 字（sha256 hex 32 字符 UI 太丑）+ score 转百分比（0.83 → "83%"）+ safe-area-inset 兜底；② **`apps/mp/utils/prospectus.ts`**（新建 +135）跨端打开 PDF 三支 — H5 `window.open(url, '_blank', 'noopener,noreferrer')` 直接外跳；MP-WEIXIN `uni.showLoading` → `uni.downloadFile`（业务域名未加白时 `errMsg` 命中 `/domain list/i` 区分文案）→ `uni.openDocument`（fileType 按 URL 后缀推断, 默认 PDF）→ 任一步 fail 静默 fallback 复制 URL + toast；APP-PLUS `plus.runtime.openURL(url)`；其他 MP 平台直接 fallback；**关键合规**：不下载到永久存储（spec/06 §法律：招股书属第三方版权材料），MP 走临时路径 + 系统组件预览正合规；③ **`apps/mp/pages/ipo/agent.vue`**（升级 +90 -25）接 CitationDrawer + chip 列表可点 + lazy-fetch — 新增 4 个 ref（`drawerVisible` / `drawerCitations` / `drawerActiveIdx` / `drawerIpoName`）+ `_prospectusCache: ref<Map<code, string|null>>`（三态：缺 key=未拉 / null=明确无原文 / string=有 URL）+ `_prospectusInflight: Set<code>`（防同一 ipo_code 并发触发多次 `fetchIPODetail`）+ `onCitationTap` 重写从 `uni.showModal` 占位改为打开抽屉 + `onEnsureProspectus(ipoCode)` 异步 fetch 写缓存（用 `new Map(...)` 替换触发 ref 响应而非 `.set` in-place）+ `drawerProspectusUrl: computed` 三态送给抽屉 + `onOpenProspectus({url, ipoName})` 调 `openProspectusUrl()`；citation chip 从纯展示升级为可点 `@tap` + `hover-class="citation-chip-hover"`（80ms hover-stay-time）+ CSS `transition: background 0.15s ease`；模板尾追加 `<CitationDrawer v-model:visible v-model:active-idx :citations :ipo-name :prospectus-url @ensure-prospectus @open-prospectus />`。**关键设计 / 取舍**：① **不走"ActionSheet → 抽屉"两步弹** — 单 chip → 单抽屉 + 抽屉内 ‹/› 切换, 关一次抽屉即可换 IPO / 离页, UX 更连贯（原 spec 字面 outdated, 实施落地版优先）；② **底部抽屉而非全屏 modal** — 占 ~75vh max 留出 chat 上下文, 用户能感知"我点的是这条消息的第 N 条引用"；③ **prospectus 按钮三态语义** — `string|null|undefined` 三态明确区分"已拉到 vs 拉完了没有 vs 还在拉"，UI 反应符合用户预期；④ **不在 store 里做 IPODetail 缓存** — 该缓存仅 chat 抽屉用, 跨页不复用（详情页有自己的拉取逻辑），短期内放页内 `ref<Map>` 是最薄方案；⑤ **`_prospectusInflight` 用普通 Set 而非 ref** — 不需要响应式（仅函数内部读写做防重判断）；⑥ **缓存写入用 `new Map(...)` 替换** — Vue 3 的 `ref<Map>` 不会自动追踪 `.set` / `.delete`；⑦ **"前/后切换"基于数组顺序而非 idx 数值** — 后端 `invalid_citation_indices` strip 后 `idx` 可能不连续（如 `[1,3,5]`），用数组 prev/next 跳转更稳定；⑧ **citation chip 30 字截断不变** — chip 本身就是预览, 点开抽屉看完整 snippet；⑨ **`openProspectusUrl` 无返回值 fire-and-forget** — 调用方不需 await, 任何错都内部 toast 兜底；⑩ **静默 fallback** — 任何打开失败都回退到"复制 URL + toast"而非红色弹窗, 避免阻塞用户感知；⑪ **不下载到永久存储** — 法务 spec/06 硬约束, MP 走临时路径 + 系统组件预览。**测试 / 质量**：vue-tsc 0 错 / ESLint 0 错（IDE LSP 全验证；`pnpm install` 因上游 `@dcloudio/uni-h5@3.0.0-4060920241225001` 被 yank 跑不动是已知历史问题, 与本 PR 无关）；端兼容 H5 + MP-WEIXIN + App 全覆盖；单测留 QA-S2-003 一并做（vitest + happy-dom）。

下一步推荐 → **FE-S2-004（VIP 升级 modal + 配额引导精修，0.5d）**：Sprint 2 前端最后一刀。BE-S2-008 配额闸门 + Retry-After 已就绪，FE 仅做"升级 modal + 文案精修 + 跳支付占位"。完成即可关 Sprint 2 进入 Sprint 3（文章聚合 + 券商对比 + VIP 订阅）。
