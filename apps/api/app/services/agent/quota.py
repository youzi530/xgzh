...
def _resolve_plan(user: User | None) -> QuotaPlan:
    """决定用户当前 plan — sync **白名单优先**版本.

    - 没登录 → ``ANONYMOUS``
    - 登录 + UUID 在 ``settings.vip_user_id_whitelist`` → ``VIP``
    - 登录 + 手机号在 ``settings.vip_user_phone_whitelist`` → ``VIP``  (dev 友好)
    - 否则 → ``FREE``  (注意: 不查 ``vip_memberships`` 表)

    BE-S3-009 起, 真实 VIP 判定路径走 :func:`_resolve_plan_with_membership`
    (async, 调用 ``vip_service.is_user_vip``). 本函数保留是为了:
    1. 单元测试覆盖白名单分支无需起 DB
    2. dev/test 环境无 vip_memberships 行也能模拟 VIP (兜底)
    3. ``check_quota`` / ``record_usage`` 内先 sync 兜底命中白名单 → 跳过 DB 查询 (热路径优化)

    手机号匹配走归一化 bare 形式 (去 +86/+852/+65 前缀); 与 ``vip_user_phone_set`` 一致.
    """
    if user is None:
...
async def _resolve_plan_with_membership(user: User | None) -> QuotaPlan:
    """决定用户当前 plan — async **接 ``vip_memberships`` 真表** 版本 (BE-S3-009).

    决策树:
    1. ``user is None`` → ``ANONYMOUS``
    2. ``_resolve_plan(user) is VIP`` (白名单兜底) → ``VIP`` (跳过 DB 查询, 热路径)
    3. ``vip_service.is_user_vip(user_id)`` (status active/trialing + end_at > now) → ``VIP``
    4. 否则 → ``FREE``

    端到端单点查 < 1ms (走 ``ix_vip_memberships_status_end_at`` + user_id UNIQUE);
    DB 异常 → fail-open 降到白名单 plan, 与"配额闸门不该挡用户"原则一致.
    """
    sync_plan = _resolve_plan(user)
    if sync_plan is not QuotaPlan.FREE:
        return sync_plan
    assert user is not None  # _resolve_plan(None) returns ANONYMOUS, 已被上面分支吃掉
    try:
        # 局部 import 避免与 vip_service 循环依赖 (vip_service 不需要 quota, 但 Python 静态扫描更稳)
        from app.services import vip_service

        if await vip_service.is_user_vip(user.user_id):
            return QuotaPlan.VIP
    except Exception as e:  # noqa: BLE001
        logger.warning(
            f"agent.quota.is_user_vip_fail user_id={user.user_id} err={e!r} (fail-open to FREE)"
        )
    return QuotaPlan.FREE


def resolve_plan(user: User | None) -> QuotaPlan:
    """sync 公共 API: **不查表**, 仅看白名单. 给单测 / 端层日志看.

    生产 / 集成场景应该用 :func:`_resolve_plan_with_membership` (async). 这个 sync
    版本保留是为了: ``test_agent_quota.py`` 单元测覆盖白名单分支不起 DB.
    """
...
async def check_quota(
    *,
    ...
    流程:
    1. ``_resolve_plan_with_membership`` 决定 plan (sync 白名单兜底 + async 查 vip_memberships 真表)
    ...
    *不抛异常*. 调用方拿 status 自己决定要不要 raise (端层一般在 ``has_quota=False``
    ...
    """
    plan = await _resolve_plan_with_membership(user)
