...
### FE-S3-002 · 文章详情 + TL;DR 底部抽屉 ✅ 2026-04-28
...
**完成情况**：详情页 hero / AI 摘要 / 关键词高亮 / 跳原文 / related_articles + TL;DR 抽屉 (饼图 / Top3 论据 / 来源列表 / insufficient_data 兜底 / 错误重试) 全部跑通 ✅；vue-tsc 新文件 0 错；MP-WEIXIN + H5 双 bundle 构建成功。

**最终落地文件**

- `apps/mp/components/SentimentPieChart.vue`（新建：纯 SVG `<circle>` + `stroke-dasharray` 实现 3 色环形饼图 — 看多绿 / 看空红 / 中性蓝灰；中心叠加文章总数 + "篇"；3 色都为 0 时显示"数据不足"灰圈兜底；底部 3 列图例 (颜色点 + 标签 + 百分比)；不引 uCharts，包体积 0 增量）
- `apps/mp/components/TldrDrawer.vue`（新建：底部抽屉 — 复用 CitationDrawer 同款 slide-up 动画 (独立 keyframe 名 `td-slide-up` 防 SCSS scope 冲突) + 4 状态分流 (loading / error / insufficient_data 兜底 / ok) + SentimentPieChart + Top3 看多 / 看空两栏 + 来源 ≤ 5 篇可点击 + 免责文案 + 生成时间；mask + close + retry + source-click 4 事件 emit）
- `apps/mp/pages/article/detail.vue`（新建：404 兜底页 + hero (logo + 来源 + ⭐ + 标题 + SentimentBadge md + 绝对时间) + 关联 IPO chip 行 + AI 摘要卡 (金色 + 关键词高亮 segment + 关键词列表) + 引导原文卡 (虚线提示 BE 不返 content) + related_articles 列表 + 底部固定双 CTA (复制链接 + 查看原文))
- `apps/mp/pages/article/index.vue`（接真：将 onArticleClick 占位 toast 改为 navigateTo detail；gotoTldr 占位 toast 改为打开 TldrDrawer + fetchTLDR 调用；切 market 时清旧 TL;DR 缓存；onTldrSourceClick 关抽屉再 navigateTo (避 transition 冲突)）
- `apps/mp/pages.json`（+ 注册 `pages/article/detail` 路由）

**关键设计决定（与初版 spec 的差异）**

1. **关键词高亮算法**: spec 说"前端 `<text>` 拼接时关键词 segment 加金色背景"。实际落地 — 在 summary 字符串里跑一次"找所有 keyword 命中 [start, end] → 排序合并重叠 → 切 segments" O(K×L) 算法 (K ≤ 10 keywords, L ≤ 200 字符 = 2000 ops, 一次性算完, 无性能压力)。每 segment 走 `kind: 'plain' | 'hl'` 两态 + computed 缓存, 渲染时 v-for 输出 `<text>` (mp-weixin 不能用 innerHTML, 必须用拼 text 段)
2. **不渲染原文 HTML**: BE 当前不返 content (合规 + 著作权 — 抓第三方文章原文存我库再展示有著作权风险), 详情页只能展示 AI 摘要 / 关键词 / 来源链接 + 跳原文按钮。这是**产品的法律护栏**不是技术限制 (spec/06 §法律隔离), 用引导卡明确告知用户"原文由 XX 提供"
3. **复制链接 + 查看原文 双 CTA**: 微信小程序无法直接打开第三方 url (web-view 域名要先备案); 实际落地"查看原文"= setClipboardData + uni.showModal 引导"已复制, 在浏览器打开"。这与 spec/06 §法律隔离 + 微信生态约束完全对齐 — 真实部署时如果第三方域名都备案了, 单点替换为跳 web-view 中转页
4. **TL;DR scope 派发策略**: BE-S3-005 不接受 scope='global', `market='all'` 的列表页用户点 TL;DR 时, 退化用 `scope=market, scope_value=HK` (港股池量大, 默认主战场)。等 BE 加 `scope=global` 全网汇总再去掉退化分支
5. **不复用 CitationDrawer 但复用动画 keyframe 名**: 内容形态完全不同 (那个是文档片段引用, 这个是汇总数据); 仅复用容器结构 + slide-up 动画。独立 keyframe 名 `td-slide-up` 与 `cd-slide-up` 防 SCSS scope 串
6. **饼图用 SVG dasharray 不用 canvas**: uCharts ~ 50KB+ gzipped, 仅 TL;DR 一处用, 性价比低; mp-weixin canvas 上小程序基础库 < 2.9 性能差 + 跨端渲染差异大。SVG `<circle>` + `stroke-dasharray = "<arc> <gap>"` + `stroke-dashoffset = -prevSum × C` 6 行写完 3 色环形, 跨 H5 / MP / App 一致
7. **来源文章 ≤ 5 篇 + 列表内反查**: TL;DR 候选池上限 30 篇, 全列爆抽屉; 显前 5 篇够用。`source_article_ids` 反查走 `list.value.filter(a => idSet.has(a.article_id))` 直接命中列表已加载文章, 不再额外打 BE — 列表通常已覆盖大部分 source (用户翻过的页), 余下少量无法显示也可接受
8. **gotoTldr reopen 不重拉**: 同 scope 下 payload 还在的 reopen 直接显已有数据, 走 BE 的人是想"刷新看看新文章" → 抽屉里点"重试"按钮走 force_refresh。两层缓存 (BE Redis 30min + 前端 ref) 让重复打开 0 网络成本
9. **onTldrSourceClick 关抽屉再 navigate**: mp-weixin 上 navigateTo 与抽屉 transition 同时进行会闪一下后又退回 (transition 触发了 navigation cancel)。`closeTldr() → setTimeout(navigate, 100)` 让 transition 先收尾再跳页
10. **detail 页 related_articles 走 redirectTo 不 navigateTo**: 用户在 article#A → 点 related → article#B → 点 related → article#C 这种链, navigateTo 会越压越深 (栈深 10), redirectTo 替换栈顶, 让用户后退仍能回到列表。微信小程序栈深 10 限制 + 用户体验"想退就退"

**对外 UI 行为（公开契约）**

| 路由 | 入参 | 行为 | 跨端 |
|---|---|---|---|
| `/pages/article/detail?article_id=XXX` | article_id | hero + AI 摘要 + 关键词高亮 + 引导原文 + related_articles + 底部双 CTA | 全端一致 |
| 详情页"查看原文" | (内部 original_url) | setClipboardData + showModal "已复制" | 全端一致 (mp-weixin web-view 限制) |
| 详情页 related card tap | (article_id) | redirectTo 同详情页 (替换栈) | 全端一致 |
| 列表页 💡 TL;DR FAB | (内部 market) | open TldrDrawer + fetchTLDR | 全端一致 |
| TL;DR 抽屉来源 tap | (article_id) | 关抽屉 + 100ms 后 navigateTo detail | 全端一致 |
| TL;DR 抽屉重试 | (内部) | fetchTLDR(force_refresh=true) | 全端一致 |

**端到端流转**

```
[列表页]
  → tap 卡片 → /pages/article/detail?article_id=XXX
  → fetchArticleDetail → 渲染 hero / 摘要 / 关键词 / 相关
  → tap "查看原文" → 复制 URL + showModal 引导
  → tap related card → redirectTo 同 detail (栈替换)
  → tap 关联 IPO chip → /pages/ipo/detail?code=00700.HK

[列表页]
  → tap 💡 TL;DR FAB → 打开 TldrDrawer
  → fetchTLDR(scope='market', scope_value='HK')
    → 200 + status='ok' → 显饼图 + Top3 + 来源
    → 200 + status='insufficient_data' → 显兜底文案
    → 5xx / 网络 → 显错误 + 重试按钮
  → tap 来源 → 关抽屉 + navigate detail
  → tap 重试 → force_refresh=true 重拉
```
...
文章线 (FE-S3-001 + 002) 已完整闭环, 用户从首页 📰 → 列表 → 详情 → TL;DR 全链路可走. 剩余前端 P0 战场: FE-S3-003 券商对比页 (依赖 BE-S3-007 + 008, BE-S3-007 已完成, 008 待确认), 完成后 P0 前端全部到位, 进入 QA-S3-001 / 002 e2e 收尾.

---

### FE-S3-003 · 券商对比页 UI ✅ 2026-04-28
...
**完成情况**：列表页 (4 维筛选 + 卡片瀑布流) + 详情页 (hero + 推广卡 + 费率明细 + 计算示例 + 平台特性 + 底部 CTA) + 跨端跳转 + UTM 归因 全部跑通 ✅；vue-tsc 新文件 0 错；MP-WEIXIN + H5 双 bundle 构建成功；spec 提的"横滚表 (sticky 首列)"主动收敛为卡片瀑布流 (详见 §关键设计 1)。

**最终落地文件**

- `apps/mp/api/broker.ts`（新建：`fetchBrokerList` / `fetchBrokerDetail` / `buildRedirectUrl` 3 个 API + 完整 ts schema —— `BrokerPublic / BrokerListResponse / Market / MarketFilter / PartnershipFilter / RedirectParams` 6 个 type，字段 100% 对齐 BE `app/schemas/broker.py`，partnership_* 三字段刻意不在 type 内 (BE schema 也无, 防意外暴露))
- `apps/mp/utils/device.ts`（新建：`getDeviceId()` —— UUIDv4 生成 + uni storage 持久化 + 模块级缓存; 与 `push_tokens.device_id` + `conversion_events.device_id` 同语义匿名稳定 key; 不依赖 `crypto.randomUUID` 走 Math.random fallback 兼容 mp-weixin JSCore)
- `apps/mp/utils/request.ts`（+ 新增 `buildAbsoluteApiUrl(path)` 公开 helper —— 把相对路径拼成完整后端 URL；BE-S3-008 redirect 端点 302 链路必须用绝对 URL (浏览器跟随 302 / mp 复制粘贴) 不能走 vite proxy 同源)
- `apps/mp/components/BrokerCard.vue`（新建：单密度卡片 — logo 80rpx + 中英名 + 市场 chip ×3 (限) + 关键费率单行 (港股佣金 / 最低 / 平台费 三栏) + 推广卡 (标题 + 描述 + 倒计时 + 过期灰色态) + 双 CTA (查看详情 secondary + 立即开户 primary))
- `apps/mp/pages/broker/index.vue`（新建：sticky market 筛选条 (4 选: 全部 / HK / US / SG) + BrokerCard 列表 + 错误 banner + 下拉刷新 + 合规底部 footer + 跨端"立即开户"分发 (H5 window.open / APP-PLUS plus.runtime.openURL / MP-WEIXIN setClipboardData))
- `apps/mp/pages/broker/detail.vue`（新建：404 兜底 + hero (logo + 中英名 + 支持市场 chip + 监管牌照 chip) + 推广卡 (标题 + 描述 + 倒计时 + 邀请码可复制) + 费率明细表 (12 字段中英映射 + 数字/布尔/字符串三态格式化) + 港股 1 万港币交易费用估算计算示例 + 平台特性 (9 字段中英映射) + 合规 footer + 底部 fixed CTA)
- `apps/mp/pages/index/index.vue`（+ hero 行加 "🏦" 圆形图标按钮跳券商对比页；与 📰 文章 / 头像并排，是 tabBar 落地前的入口）
- `apps/mp/pages.json`（+ 注册 `pages/broker/index` 与 `pages/broker/detail` 两条路由）

**关键设计决定（与初版 spec 的差异）**

1. **横滚表收敛为卡片瀑布流**: spec 提"横滚表 + 首列冻结"，但实测：
   - mp-weixin scroll-view 嵌套 + position sticky 兼容性差 (基础库 ≥ 2.7 才稳, < 2.7 会 first-cell 跟着滚)
   - 6-8 家券商 × 7+ 维度的横滚表小屏阅读体验 < 单卡片完整展示 (用户反复横滚找一个 broker 的某一项)
   - 卡片已能横向并列 3 项关键费率 (佣金 / 最低 / 平台费), 满足"快速对比"核心需求
   - 真正深度对比走详情页 (12 字段 + 计算示例 + 平台特性), 比表格挤压字号阅读舒服
   - 横滚表可作为 P1 优化 (做个独立 "对比" tab toggle), 现在不阻塞转化路径
2. **跨端"立即开户"三端分发**: spec/06 §法律隔离要求 + 微信生态约束 → 硬编 3 端逻辑:
   - **H5**: ``window.open(url, '_blank')`` 让浏览器跟随 BE 302
   - **APP-PLUS**: ``plus.runtime.openURL(url)`` 唤起系统浏览器 (uni-app 跨端 API)
   - **MP-WEIXIN**: ``setClipboardData + uni.showModal`` 引导用户在浏览器粘贴打开 (mp-weixin web-view 不能跳任意域名; 真实部署时如果券商域名都备案了, 可单点替换为 navigateTo `/pages/webview/external?url=...`)
   - 所有路径都经 BE ``/redirect`` 端点 (而非前端拼 referral_url 自跳), 保证 conversion_events.click 落库, 不丢 CPA 漏斗第一步
3. **UTM 归因双轨制**: 列表页 ``utm_campaign='compare_table'``, 详情页 ``utm_campaign='detail_cta'``; BE-S3-008 conversion_events 按 (broker_id, actor_key, utm_campaign) 三元组防刷, 即"列表 click → 1h 内进详情再 click" 的路径会落 2 行 (compare_table + detail_cta), 让运营漏斗能区分"看了详情才开户" vs "列表直接开户" 的转化路径差异
4. **device_id UUIDv4 + storage 持久化**: 匿名用户跨页面 / 跨次启动稳定的去重 key, 与 `push_tokens.device_id` 同源; 模块级 _cached + storage 双层让 ``getDeviceId()`` 调用 0 网络 0 异步; 不用 ``crypto.randomUUID()`` 是因为 mp-weixin JSCore 不一定有, Math.random 弱熵对去重场景够用
5. **`buildAbsoluteApiUrl` 公开 helper**: 普通 JSON API 走 `request()` 内部 baseURL 隐式拼接 OK; 但 redirect 端点需要前端拿到完整 URL (浏览器跟随 302 / mp 复制粘贴) — 加一个公开 helper 让上层显式调用, 不污染 ``request()`` 主路径
6. **partnership 字段不放筛选**: BE 接受 `partnership=CPA/CPS/BOTH/NONE` 筛选, 但端侧用户对"我们与券商的合作类型"完全不需要关心 (商业秘密); 前端永远走 `partnership=all`, UI 上不暴露这个维度
7. **`market='A'` 筛选不放**: 6 家种子券商暂不支持 A 股直连 (国内券商接入通常走另一套合规体系), 筛选只放 HK/US/SG + 全部 4 选; 等实际接入 A 股券商再加分支
8. **BrokerCard 推广倒计时颗粒度**: ≥ 7 天显 "还剩 X 天"; 1-7 天显 "还剩 X 天 Y 时"; < 1 天显 "X 小时内截止"; 过期显灰色 "已结束". 与 me/index.vue VIP 卡同款颗粒度, 维持视觉一致性
9. **fees / features key 走 dict 中英映射 + 白名单顺序**: BE seed 各券商可能加新 key (例如 ``us_commission_per_share``), 前端 dict 兜底 = 直接显 raw key 让运营/QA 能看出"这是新加的字段, FE 没翻译"; 不抛 i18n missing key 报错。同时通过 `Object.keys(LABELS).filter(k => k in fees).map(...)` 走"白名单 + 顺序固定"渲染, 防各券商字段顺序不一造成的 UI 抖动
10. **港股 1 万港币交易费用估算**: 用户最直观的对比基准 — `max(rate × 10000, min_commission) + platform_fee`, 不做交互式计算器 (那是 P1 工具, MVP 简单示例足够); note 明确"不含交易所费用 / 印花税 / 转手纸费 / 政府征费", 防被法务挑刺"误导用户"

**对外 UI 行为（公开契约）**

| 路由 | 入参 | 行为 | 跨端 |
|---|---|---|---|
| `/pages/broker/index` | none | 4 维 market 筛选 + 卡片瀑布流 + 下拉刷新 | 全端一致 |
| 列表页"立即开户" | (内部 slug) | utm=compare_table, 跨端分发 BE redirect | H5 window.open / APP plus / MP 复制 |
| `/pages/broker/detail?slug=XXX` | slug | hero + 推广 + 费率 + 计算示例 + 特性 + 底部 CTA | 全端一致 |
| 详情页"立即开户" | (内部 slug) | utm=detail_cta, 跨端分发 BE redirect | H5 window.open / APP plus / MP 复制 |
| 详情页推广卡邀请码 tap | (内部 invite_code) | setClipboardData + toast | 全端一致 |
| 首页 🏦 图标 | (内部) | 跳 `/pages/broker/index` | 全端一致 |

**端到端流转 (BE-S3-008 集成)**

```
[列表页 立即开户]
  → buildRedirectUrl(slug, {utm_campaign='compare_table', utm_medium='compare-page', device_id=XXX})
  → H5: window.open(BE_URL + '/api/v1/brokers/futubull/redirect?utm_campaign=compare_table&...')
  → BE 端点:
    → conversion_service.get_active_broker_by_slug(slug) → broker
    → conversion_service.build_redirect_url(broker, ...) → futunn.com/...?utm_source=xgzh&utm_campaign=compare_table&invite_code=XGZH-FUTU
    → conversion_service.log_click_with_dedup(broker, actor_key='ip:1.2.3.4' / 'd:UUID', utm_campaign='compare_table')
      → Redis incr_with_expire conversion:click:<broker_id>:<actor_key>:compare_table TTL=3600s
      → 第一次 → INSERT conversion_events (event_type='click', utm_campaign='compare_table', ...)
      → 第二次起 → 跳过落库, 但仍 302
    → 302 → futunn.com 落地页 (用户在券商方完成 KYC)
  → 用户在券商方完成入金 / 首笔交易后 → 券商 Postback (Sprint 4+) → INSERT conversion_events (event_type='deposit', attributed=True, amount_cny=XXX)

[详情页 立即开户]
  → 同上, 但 utm_campaign='detail_cta', utm_medium='detail-page'
  → BE 端点 dedup key 不同 (不同 utm_campaign 各落 1 行), 漏斗能 GROUP BY 区分两条转化路径
```

**质量门**

- ✅ vue-tsc 新文件 0 错（pre-existing 错位于 `pages/ipo/detail.vue` / `utils/sse.ts` + `request.ts` 第 122 行 — 后者与本 PR 加的 `buildAbsoluteApiUrl` 无关, 是 `uni.request data: opts.data` 的 unknown 类型推断问题, 历史遗留; 行号因新增加 helper 函数从 97 → 122 移位, 但错误本身一字未改）
- ✅ `npx uni build -p mp-weixin` 构建成功
- ✅ `npx uni build` (h5) 构建成功

**下一步 (QA-S3-001 / QA-S3-002 / 横滚表 P1 优化)**

至此 **Sprint 3 P0 前端全部到位** (FE-S3-001/002/003/004/005), 用户从注册 → 试用 → 文章 / 券商 / IPO 三条业务线探索 → 撞 quota → 升级 → 支付 → 续费 + CPA 跳转 → 漏斗归因 全链路跑通. 剩余战场:
1. **QA-S3-001** 文章流水线 e2e (依赖 BE-S3-006, 已就绪): 6 条用例覆盖 ingest → dedup → sentiment → tldr → 列表 / 详情 / 搜索全链路
2. **QA-S3-002** 微信支付沙箱 e2e (依赖 BE-S3-010 + 微信商户号): 5 条用例覆盖注册 → 试用 → 升级下单 → 沙箱回调 → 配额放开 → 续费堆叠
3. **横滚表 P1 优化** (非必需): 在券商对比页加 toggle "卡片视图 / 表格视图", 表格视图实现 sticky 首列横滚表; 现 MVP 不阻塞
...
### FE-S3-002 ✅ 2026-04-28（实施成果详见上方主体段）

### FE-S3-003 ✅ 2026-04-28（实施成果详见上方主体段）
