...
| 候选 | 理由 |
|------|------|
| **BE-S2-004** (招股书 PDF 解析 + 切分 + 入库, 1d) | 三依赖全到位：BE-S2-000 提供 `prospectus_url`，BE-S2-002 提供 embedding facade，BE-S2-003 提供 `document_chunks` 表。RAG 主线最关键的一步，把外部数据真正灌进检索层 |
| BE-S2-005 (混合检索 + RRF + reranker, 1d) | 已可起（依赖 BE-S2-003 满足）；但没有 BE-S2-004 灌的 chunks 跑出来召回@5 没法测，先把 BE-S2-004 做完再做检索 |
| BE-S2-006a (Tool 注册中心 + 2 工具, 1d) | 已可起；BE-S2-007 才真用 Tool Use，可以并行起，但 RAG 主线优先 |

→ **建议下一步走 BE-S2-004**（招股书入库；spec 里 RAG 主线 BE-S2-000 ✅ → **BE-S2-004** → BE-S2-005 → BE-S2-006 → BE-S2-007 推进。BE-S2-004 PR 内会落地：PDF 下载 + 解析 + 语义切分 + 1024-d embedding 批量入 `document_chunks` + 入库幂等 + 失败兜底）
